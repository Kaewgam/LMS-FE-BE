...
// --- Mock Data: รายชื่อ 'นักเรียน' ---
const chatStudents: { [key: string]: ChatPartner } = {
    "1": {
        name: "สมศรี มีชัย",
...
// --- Mock Data: บทสนทนาตัวอย่าง ---
const initialMessages: Message[] = [
    { id: 1, sender: 'partner', text: "อาจารย์คะหนูส่งงานไม่ได้ค่ะ หนูกดส่งไปแล้วแต่งานไม่ขึ้นค่ะ", timestamp: "11:02 น." },
    { id: 2, sender: 'me', text: "เดี๋ยวอาจารย์ตรวจสอบให้ค่ะ", timestamp: "12:00 น.", status: 'read' },
    { id: 3, sender: 'partner', text: "ขอบคุณมากค่ะอาจารย์", timestamp: "12:01 น." }
];

// --- Main Chat Page Component (สำหรับผู้สอน) ---
export default function ChatPageTeacher() {
    const searchParams = useSearchParams();
    const studentId = searchParams.get('studentId') || "1";
    const isNewChat = searchParams.has('studentId');
    const chatPartner = chatStudents[studentId as keyof typeof chatStudents] || chatStudents["1"];
    ...
    return (
        <main className="max-w-4xl mx-auto mt-6 bg-white rounded-xl border-1 border-gray-300 shadow-lg flex flex-col h-[85vh]">
            <header className="flex items-center p-4 border-b rounded-t-xl border-gray-300 sticky top-0 bg-white z-10">
                <Link href="/instructor/all_notifications" className="p-2 rounded-full hover:bg-gray-100"><ArrowLeft size={24} /></Link>
                    ...
                    <>
                        ...
                        {messages.map((msg) => (
                            <div key={msg.id} className={`flex items-end gap-2 w-full ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-xs md:max-w-md p-3 rounded-2xl ${msg.sender === 'me' ? 'bg-green-600 text-white rounded-br-none' : 'bg-gray-200 rounded-bl-none'}`}>
                                    <p className="break-words">{msg.text}</p>
                                </div>
                                {msg.sender === 'partner' && <p className="text-xs text-gray-600 self-end mb-1">{msg.timestamp}</p>}
                                {msg.sender === 'me' && <Image src="/images/teacher-avatar.png" alt="You" width={28} height={28} className="rounded-full" />}
                            </div>
                        ))}
                    </>
                ) : (
