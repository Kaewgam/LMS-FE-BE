...
// ---------- Config ----------
const BASE = '/universities-staff/listorganizations/';
    ...
    <div className="fixed inset-0 flex justify-center items-center z-50 bg-black/40 p-4">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md border border-gray-200">
        <div className="flex justify-between items-center border-b pb-3 mb-4">
          ...
          <button onClick={onClose} className="cursor-pointer" disabled={disabled}>
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center z-50 bg-black/40 p-4">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md border border-gray-300">
        ...
        <div className="mb-4">
          <label htmlFor="orgName" className="block text-sm font-medium">ชื่อองค์กร</label>
          <input
            type="text"
            id="orgName"
            value={orgName}
...
interface Organization {
  id: string;      // ถ้า backend เป็น number เปลี่ยนได้
  name: string;
  created_at?: string;
}
...
const OrganizationManagementPage: React.FC = () => {
  ...
  async function createOrganization(name: string) {
    try {
      ...
      setOrganizations(prev => [res.data as Organization, ...prev]);
  ...
  async function updateOrganization(id: string, name: string) {
    try {
      const res = await api.patch(`${BASE}${id}/`, { name });
      setOrganizations(prev => prev.map(o => (o.id === id ? (res.data as Organization) : o)));
  ...
  async function deleteOrganization(id: string) {
    try {
      ...
      toast.success('ลบองค์กรเรียบร้อยแล้ว');
    } catch (e: any) {
  ...
  const handleSaveEdit = async (orgId: string) => {
    ...
    if (!newName) return toast.error('ชื่อองค์กรต้องไม่ว่างเปล่า');
    setLoading(true);
    try {
      await updateOrganization(orgId, newName);
  ...
  const handleAddOrganization = async (name: string) => {
    ...
    if (isDuplicate) return toast.error('ไม่สามารถเพิ่มได้ ชื่อองค์กรนี้มีอยู่ในระบบแล้ว');
    setLoading(true);
    try {
    ...
    <div className="min-h-screen p-4 sm:p-8 font-sans antialiased">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">รายชื่อองค์กรทั้งหมด</h1>
          <button
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-[14px] font-semibold uppercase tracking-wider border-r border-gray-300">ชื่อองค์กร</th>
                <th className="px-4 py-3 text-center text-[14px] font-semibold uppercase tracking-wider">การจัดการ</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {loading && organizations.length === 0 ? (
                <tr><td colSpan={3} className="px-6 py-10 text-center text-lg">กำลังโหลด...</td></tr>
                  ...
                  <tr key={org.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-center text-sm">
                      <div className="flex items-center justify-center gap-6">
                        {editingOrgId === org.id ? (
                          <>
                            <button onClick={() => handleSaveEdit(org.id)}
                ...
                <tr><td colSpan={3} className="px-6 py-10 text-center text-lg">ไม่พบข้อมูลองค์กร</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="md:hidden flex flex-col gap-4">
          {loading && organizations.length === 0 ? (
            <div className="bg-white rounded-lg shadow-md p-6 text-center text-lg">กำลังโหลด...</div>
          ) : filteredOrganizations.length > 0 ? (
            filteredOrganizations.map((org, index) => (
              <div key={org.id} className="bg-white rounded-lg shadow-md p-4 space-y-2 border border-gray-300">
                ...
                <div className="text-[13px]">
                  <div className="grid grid-cols-2 gap-y-1">
                    <div><span className="font-semibold">ลำดับ :</span> {index + 1}</div>
                  </div>
                </div>
                <div className="pt-2 border-t border-gray-300 flex justify-end gap-3 mt-4">
                  {/* reuse the same buttons */}
                  <div className="w-full flex justify-end">{renderCardButtons(org)}</div>
                </div>
              </div>
            ))
          ) : (
            <div className="bg-white rounded-lg shadow-md p-6 text-center text-lg">ไม่พบข้อมูลองค์กร</div>
          )}
        </div>
      </div>
    </div>
