...
const initialNotifications: Notification[] = [
    { id: 1, type: 'q&a', sender: "ริสา บุญนาค", avatar: "/images/40.png", time: "12.00 น.", relativeTime: "เมื่อ 10 นาทีที่แล้ว", lastMessage: "เดี๋ยวอาจารย์ตรวจตอบให้ค่ะ", status: "unread", href: "/student/q&a/chat/1" },
    { id: 2, type: 'broadcast', sender: "เอเดน มิลเลอร์", avatar: "/images/40.png", time: "11.00 น.", relativeTime: "เมื่อ 1 ชั่วโมงที่แล้ว", title: "แจ้งเรื่องสำคัญ", status: "read", href: "/student/notifications_student/details/2" },
    { id: 3, type: 'broadcast', sender: "LMS", avatar: "/images/40.png", time: "10.00 น.", relativeTime: "เมื่อ 2 ชั่วโมงที่แล้ว", title: "อัปเดตระบบ", status: "read", href: "/student/notifications_student/details/3" }
];

const menuItems = [
    { key: 'notifications', label: 'การแจ้งเตือนทั้งหมด', href: '/student/all_notifications' },
    { key: 'send-messages', label: 'การส่งข้อความแจ้งเตือน', href: '#' },
    { key: 'q-and-a', label: 'Q&A กับผู้สอน', href: '#' },
];

const instructors = [
    { id: 1, name: "ริสา บุญนาค" }, { id: 2, name: "เอเดน มิลเลอร์" }, { id: 3, name: "สมชาย ใจดี" },
];
const initialQnaHistory = initialNotifications.filter(noti => noti.type === 'q&a' && noti.sender === "ริสา บุญนาค");

const additionalRecipients = [
    'ผู้เรียน : คอร์สภาษาอังกฤษ ป.4', 'ผู้สอน : คอร์สภาษาอังกฤษ ป.6', 'ผู้ดูแลสถาบัน', 'ผู้สอน : คอร์สคณิตศาสตร์ ม.1'
];
...
export default function AllNotificationsPage() {
    ...
    // States for Send Message Form
    const [recipientType, setRecipientType] = useState<'all-users' | 'all-teachers' | 'all' | 'other'>('all-users');
            ...
            <div className="max-w-5xl mx-auto bg-white border border-gray-300 shadow-lg rounded-2xl flex flex-col md:flex-row overflow-hidden">
                ...
                <section className="flex-1 p-6 sm:p-8">
                    ...
                    {activeMenuItem === 'q-and-a' && (
                        <>
                           <header className="pb-4 border-b border-gray-300"><h1 className="text-2xl font-semibold ">Q&A กับผู้สอน</h1></header>
                           <div className="relative mt-6" ref={comboRef}>
                               <div className="relative"><input type="text" value={comboQuery} onChange={(e) => setComboQuery(e.target.value)} onFocus={() => setIsComboOpen(true)} placeholder="ค้นหาผู้สอนที่ต้องการสนทนา" className="w-full bg-white border border-gray-300  py-3 px-4 pr-8 rounded-xl leading-tight focus:outline-none focus:ring-1 focus:ring-gray-300" /><div className=" cursor-pointer absolute inset-y-0 right-0 flex items-center px-2 "><ChevronDown size={20}  /></div></div>
                               {isComboOpen && (<div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-xl shadow-lg max-h-60 overflow-y-auto">{filteredInstructors.length > 0 ? (filteredInstructors.map((instructor) => (<div key={instructor.id} onClick={() => { setComboQuery(instructor.name); setIsComboOpen(false); const existingChat = initialNotifications.find(noti => noti.sender === instructor.name && noti.type === 'q&a'); if (existingChat) {setDisplayedChats([existingChat]);} else {const newChat: Notification = {id: Date.now(), type: 'q&a', sender: instructor.name, avatar: '/images/40.png', time: '', relativeTime: 'เริ่มการสนทนาใหม่', lastMessage: `คลิกเพื่อเริ่มต้นการสนทนากับ ${instructor.name}`, status: 'read', href: `/student/q&a/chat/new?instructorId=${instructor.id}`}; setDisplayedChats([newChat]);}}} className="px-4 py-2 hover:bg-gray-100 cursor-pointer">{instructor.name}</div>))) : (<div className="px-4 py-2 ">ไม่พบผู้สอน</div>)}</div>)}
                           </div>
                    ...
                    {activeMenuItem === 'send-messages' && (
                        <>
                            <header className="pb-4 border-b border-gray-300">
                                <h1 className="text-2xl font-semibold ">การส่งข้อความแจ้งเตือน</h1>
                            </header>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-6 gap-y-4">
                                        <div className="flex items-center gap-2">
                                            <input type="radio" id="all-users" name="recipientType" checked={recipientType === 'all-users'} onChange={() => { setRecipientType('all-users'); setSelectedRecipient(''); setIsDropdownOpen(false); setInputValue(''); }} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"/>
                                            <label htmlFor="all-users" className="font-medium">ผู้ดูแลสถาบัน</label>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <input type="radio" id="all-teachers" name="recipientType" checked={recipientType === 'all-teachers'} onChange={() => { setRecipientType('all-teachers'); setSelectedRecipient(''); setIsDropdownOpen(false); setInputValue(''); }} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"/>
                                            <label htmlFor="all-teachers" className="font-medium">ผู้สอนทุกคน</label>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <input type="radio" id="all" name="recipientType" checked={recipientType === 'all'} onChange={() => { setRecipientType('all'); setSelectedRecipient(''); setIsDropdownOpen(false); setInputValue(''); }} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"/>
                                            <label htmlFor="all" className="font-medium">ทั้งหมด</label>
                                        </div>
                                        <div className="flex items-center gap-2">
                                    ...
                                    <div className="relative w-full max-w-md">
                                        ...
                                        {isDropdownOpen && (
                                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                                {filteredRecipients.length > 0 ? (filteredRecipients.map((option) => (<div key={option} onClick={() => { setSelectedRecipient(option); setInputValue(option); setIsDropdownOpen(false); }} className="px-3 py-2 hover:bg-gray-100 cursor-pointer">{option}</div>))) : (<div className="px-3 py-2 ">ไม่พบข้อมูล</div>)}
                                            </div>
                                        )}
                                    </div>
                                )}
