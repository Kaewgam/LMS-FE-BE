...
import api, { API_BASE } from "@/lib/api"; // ถ้า alias ของโปรเจกต์ใช้ "@/lib/api" ให้เปลี่ยนเป็น: from '@/lib/api'
...
function formatDate(iso: string | null | undefined) {
  ...
  // ถ้าต้องการปี ค.ศ. ให้ใช้ตัวเลือก B ด้านล่างแทน
...
const ImportantDocumentsPage: React.FC = () => {
  ...
  const handleSaveEdit = async (docId: number) => {
    ...
    try {
      await api.patch(`/admin/documents/${docId}/`, { name: editingDocName });
      toast.success("แก้ไขเอกสารเรียบร้อยแล้ว");
      setDocuments((prev) =>
        prev.map((d) => (d.id === docId ? { ...d, name: editingDocName } : d))
      );
      setEditingDocId(null);
    } catch (e: any) {
      console.error(e);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {documents.map((doc, index) => (
                    <tr
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-center text-sm border-r border-gray-300">{index + 1}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-center text-sm border-r border-gray-300">{formatDate(doc.created_at)}
                      </td>
              ...
              {documents.map((doc, index) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-xs text-gray-500 block">{formatDate(doc.created_at)}</span>
                  </div>
